...
class InMemoryFakeDB:
    ...
    def write(self, unique_id, borrower, owner, money, note):
        ...
        record_his = self.records.setdefault(unique_id, [])
        self.records[unique_id].append((borrower, owner, money, note))

        summary_dic = self.summary.setdefault(unique_id, {})
        summary_key = (borrower, owner)
        balance = summary_dic.setdefault(summary_key, 0)
        balance -= int(money)
        self.summary[unique_id][summary_key] = balance

        summary_key = (owner, borrower)
        balance = summary_dic.setdefault(summary_key, 0)
        balance += int(money)
        self.summary[unique_id][summary_key] = balance

        # 3. Return the latest balance_number
        return (borrower, owner, balance)
